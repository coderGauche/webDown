...
> 建立日期：2026-07-22  
> 当前阶段：M5 下载引擎与权限策略
> 当前状态：M5-T4 已完成，等待开始 M5-T5
> 下一任务：M5-T5 实现超时、重试和 Retry-After
...
| 当前里程碑 | M5 下载引擎与权限策略 |
| 当前任务 | 无进行中任务 |
| 最近完成 | M5-T4 实现 AbortController 暂停、取消基础 |
| 下一任务 | M5-T5 实现超时、重试和 Retry-After |
| 阻塞项 | 无 |
...
下一步严格只执行 **M5-T5：实现超时、重试和 Retry-After**。

M5-T5 完成后必须：

1. 更新本文件中的任务勾选；
2. 建立单次请求超时和有上限的重试策略，只对明确可重试结果继续尝试；
3. 正确解析 `Retry-After` 秒数和 HTTP 日期，并在服务器建议与本地退避策略之间采用安全等待；
4. 覆盖成功、超时、可重试/不可重试失败、重试耗尽、Retry-After 与外部中止，不提前实现 M5-T6 响应元数据持久化。
