...
> 当前状态：M2-T1 已完成，等待开始 M2-T2
> 下一任务：M2-T2 定义任务状态机和合法状态转换
...
| 当前任务 | 无进行中任务 |
| 最近完成 | M2-T1 定义 `CaptureJob`、`CaptureSettings`、`ResourceRecord` |
| 下一任务 | M2-T2 定义任务状态机和合法状态转换 |
| 阻塞项 | 无 |
...
下一步严格只执行 **M2-T2：定义任务状态机和合法状态转换**。

M2-T2 完成后必须：

1. 更新本文件中的任务勾选；
2. 将 M2-T3 设为下一任务；
3. 在完成日志中记录状态转换规则、测试命令和验证结果；
4. 不越过 M2-T2 提前实现版本化消息协议或持久化。
