...
> 建立日期：2026-07-22  
> 当前阶段：M5 下载引擎与权限策略
> 当前状态：M5-T1 已完成，等待开始 M5-T2
> 下一任务：M5-T2 实现第三方域名汇总和按需授权
...
下一步严格只执行 **M5-T2：实现第三方域名汇总和按需授权**。

M5-T2 完成后必须：

1. 更新本文件中的任务勾选；
2. 从资源图中只汇总可网络抓取且不同于页面 hostname 的第三方 scheme + hostname 权限模式；
3. 区分已授权和待授权第三方站点，保持资源数量及来源可解释，并仅请求用户明确选择的模式；
4. 覆盖同站、子域、跨 scheme、重复域名、端口和非网络资源，不提前执行 M5-T3 的并发下载队列。
