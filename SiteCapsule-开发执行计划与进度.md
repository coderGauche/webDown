...
> 建立日期：2026-07-22  
> 当前阶段：M5 下载引擎与权限策略
> 当前状态：M5-T9 已完成，等待开始 M5-T10
> 下一任务：M5-T10 使用多域 fixture 做集成测试
...
下一步严格只执行 **M5-T10：使用多域 fixture 做集成测试**。

M5-T10 完成后必须：

1. 更新本文件中的任务勾选；
2. 建立不依赖公网的多 origin HTTP fixture，覆盖主站、已授权第三方、未授权第三方、重定向、429/503、超限与部分资源失败；
3. 将权限、安全策略、受控并发、重试、响应元数据、体积限制和批次聚合贯穿同一测试流程，验证下载结果与资源记录一致；
4. 证明 M5 验收门禁全部满足并记录残余网络限制，不提前实现 M6 路径映射和内容改写。
