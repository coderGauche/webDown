...
> 建立日期：2026-07-22  
> 当前阶段：M4 资源发现与资源图
> 当前状态：M4-T8 已完成，等待开始 M4-T9
> 下一任务：M4-T9 为复杂 URL、srcset、嵌套 CSS 编写测试
...
下一步严格只执行 **M4-T9：为复杂 URL、srcset、嵌套 CSS 编写测试**。

M4-T9 完成后必须：

1. 更新本文件中的任务勾选；
2. 为相对 URL、默认端口、点路径、编码路径、fragment 和保留 query 的组合行为增加综合测试；
3. 覆盖 `srcset` 中 data URL、descriptor、空白及多个候选与资源合并/来源追踪的协同行为；
4. 覆盖嵌套 CSS、`@import`、`@font-face`、转义 URL、重复发现和冲突证据，不提前执行 M5 资源下载任务。
