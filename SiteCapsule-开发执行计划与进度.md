...
> 当前状态：M1 扩展工程骨架已完成，等待开始 M2-T1
> 下一任务：M2-T1 定义 `CaptureJob`、`CaptureSettings`、`ResourceRecord`
...
| 当前任务 | 无进行中任务 |
| 最近完成 | M1-T8 在 Chromium 中加载未打包扩展并保存验证截图 |
| 下一任务 | M2-T1 定义 `CaptureJob`、`CaptureSettings`、`ResourceRecord` |
| 阻塞项 | 无 |
...
下一步严格只执行 **M2-T1：定义 `CaptureJob`、`CaptureSettings`、`ResourceRecord`**。

M2-T1 完成后必须：

1. 更新本文件中的任务勾选；
2. 将 M2-T2 设为下一任务；
3. 在完成日志中记录类型定义、测试命令和验证结果；
4. 不越过 M2-T1 的类型边界提前实现状态机或持久化。
