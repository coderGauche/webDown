...
> 建立日期：2026-07-22  
> 当前阶段：M5 下载引擎与权限策略
> 当前状态：M5-T7 已完成，等待开始 M5-T8
> 下一任务：M5-T8 实现协议、origin 和本地网络安全策略
...
下一步严格只执行 **M5-T8：实现协议、origin 和本地网络安全策略**。

M5-T8 完成后必须：

1. 更新本文件中的任务勾选；
2. 仅允许经规范化且无凭据的 HTTP/HTTPS 网络资源，对原始 URL、每个已观测重定向目标和 final URL 重新执行同一策略；
3. 拒绝 localhost、回环、未指定、链路本地、私有、组播及其 IPv4/IPv6 表示，并要求目标 origin 已获得精确的用户授权；
4. 覆盖协议/凭据、大小写与默认端口、IPv4/IPv6 本地网络边界、子域授权隔离、重定向越权和 final URL 复查，不提前实现 M5-T9 资源失败聚合策略。
