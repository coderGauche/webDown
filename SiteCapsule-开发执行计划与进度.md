...
> 建立日期：2026-07-22  
> 当前阶段：M3 当前页面 DOM 快照
> 当前状态：M2 已完成，等待开始 M3-T1
> 下一任务：M3-T1 获取当前标签页 URL、标题、base URL 和最终 URL
...
下一步严格只执行 **M3-T1：获取当前标签页 URL、标题、base URL 和最终 URL**。

M3-T1 完成后必须：

1. 更新本文件中的任务勾选；
2. 为页面元数据定义明确类型，并通过现有 Side Panel -> Background -> Content 链路返回；
3. 区分请求 URL、文档最终 URL 和 `document.baseURI`，为后续相对资源解析提供可信基准；
4. 添加正常页面与异常消息的测试，不提前实现 M3-T2 渲染等待或 M3-T3 DOM 序列化。
