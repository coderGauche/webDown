...
> 建立日期：2026-07-22  
> 当前阶段：M3 当前页面 DOM 快照
> 当前状态：M3-T6 已完成，等待开始 M3-T7
> 下一任务：M3-T7 实现超时、导航变化和标签关闭错误
...
| 当前任务 | 无进行中任务 |
| 最近完成 | M3-T6 收集 Performance Resource Timing |
| 下一任务 | M3-T7 实现超时、导航变化和标签关闭错误 |
| 阻塞项 | 无 |
...
下一步严格只执行 **M3-T7：实现超时、导航变化和标签关闭错误**。

M3-T7 完成后必须：

1. 更新本文件中的任务勾选；
2. 为页面信息请求建立明确超时边界，并将超时、捕获期间导航和标签关闭分别映射为稳定结构化错误；
3. 在返回成功快照前确认标签页仍存在且捕获起始 URL 或文档身份未变化，避免归档错误页面；
4. 覆盖正常完成、超时、导航变化、标签关闭和迟到响应测试，不提前实现 M3-T8 静态页与 SPA fixture。
