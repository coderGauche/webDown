...
> 建立日期：2026-07-22  
> 当前阶段：M3 当前页面 DOM 快照
> 当前状态：M3-T5 已完成，等待开始 M3-T6
> 下一任务：M3-T6 收集 Performance Resource Timing
...
| 当前任务 | 无进行中任务 |
| 最近完成 | M3-T5 记录 iframe、Shadow DOM 和不可访问区域 |
| 下一任务 | M3-T6 收集 Performance Resource Timing |
| 阻塞项 | 无 |
...
下一步严格只执行 **M3-T6：收集 Performance Resource Timing**。

M3-T6 完成后必须：

1. 更新本文件中的任务勾选；
2. 在渲染等待完成后读取 Resource Timing 条目，并转换为稳定、可校验的资源发现记录；
3. 只保留后续资源抓取所需的 URL、发起类型、时间和可用大小信息，不采集响应正文或请求凭据；
4. 覆盖空列表、重复 URL、跨源零大小、非法条目和类型映射测试，不提前实现 M3-T7 超时、导航变化与标签关闭错误。
