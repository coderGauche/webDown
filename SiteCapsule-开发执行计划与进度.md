...
> 建立日期：2026-07-22  
> 当前阶段：M4 资源发现与资源图
> 当前状态：M4-T5 已完成，等待开始 M4-T6
> 下一任务：M4-T6 实现 `ResourceGraph` 及来源追踪
...
| 当前里程碑 | M4 资源发现与资源图 |
| 当前任务 | 无进行中任务 |
| 最近完成 | M4-T5 实现 URL 规范化、fragment 清理和 query 保留 |
| 下一任务 | M4-T6 实现 `ResourceGraph` 及来源追踪 |
| 阻塞项 | 无 |
...
下一步严格只执行 **M4-T6：实现 `ResourceGraph` 及来源追踪**。

M4-T6 完成后必须：

1. 更新本文件中的任务勾选；
2. 定义可严格校验的 `ResourceGraph` 节点、边及来源追踪模型，以 M4-T5 规范化 URL 作为资源节点身份；
3. 将 DOM、SVG、CSS 和 Performance 的发现证据转换为可追溯边，保留来源通道、源序号和原始候选；
4. 为节点去重、边稳定排序、重复来源和损坏图数据编写测试，不提前实现 M4-T7 的协议分类与过滤。
