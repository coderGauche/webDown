...
> 建立日期：2026-07-22  
> 当前阶段：M4 资源发现与资源图
> 当前状态：M4-T4 已完成，等待开始 M4-T5
> 下一任务：M4-T5 实现 URL 规范化、fragment 清理和 query 保留
...
| 当前里程碑 | M4 资源发现与资源图 |
| 当前任务 | 无进行中任务 |
| 最近完成 | M4-T4 合并 DOM、CSS 和 Performance 资源 |
| 下一任务 | M4-T5 实现 URL 规范化、fragment 清理和 query 保留 |
| 阻塞项 | 无 |
...
下一步严格只执行 **M4-T5：实现 URL 规范化、fragment 清理和 query 保留**。

M4-T5 完成后必须：

1. 更新本文件中的任务勾选；
2. 建立统一 URL 规范化函数，明确绝对化、大小写、默认端口、路径和 percent-encoding 行为；
3. 对网络资源移除 fragment 但完整保留 query，使规范化后 URL 作为合并键，仍保留每条原始证据；
4. 为默认端口、相对路径、query、fragment、percent-encoding 和不同规范化结果编写测试，不提前实现 M4-T6 的 `ResourceGraph`。
