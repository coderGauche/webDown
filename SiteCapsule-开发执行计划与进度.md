...
> 建立日期：2026-07-22  
> 当前阶段：M5 下载引擎与权限策略
> 当前状态：M4 已完成，等待开始 M5-T1
> 下一任务：M5-T1 实现当前站点权限检查
...
| 当前里程碑 | M5 下载引擎与权限策略 |
| 当前任务 | 无进行中任务 |
| 最近完成 | M4-T9 为复杂 URL、srcset、嵌套 CSS 编写测试 |
| 下一任务 | M5-T1 实现当前站点权限检查 |
| 阻塞项 | 无 |
...
下一步严格只执行 **M5-T1：实现当前站点权限检查**。

M5-T1 完成后必须：

1. 更新本文件中的任务勾选；
2. 基于当前标签页 URL 生成最小 origin 权限模式，不使用永久全站权限替代当前站点判断；
3. 区分已授权、未授权和浏览器受限页面，并向 Background/Side Panel 提供可解释结果；
4. 为 HTTP、HTTPS、端口、子域和受限协议编写测试，不提前执行 M5-T2 的第三方域名汇总与授权流程。
