...
| B-002 | 2026-08-03 | 阻塞 | 运行时未生成 `_sitecapsule/report.html` 和完整资源清单，用户看不到失败、跳过、外链或删除原因 | P0：报告率 0%，无法区分完整归档和 HTML 壳 | M10-R5 使所有完成 ZIP 生成可读报告与结构化元数据，并由 E2E 校验 |
...
| B-004 | 2026-08-03 | 已修复待用户复验 | 大 ZIP 交给 Chrome 后立即撤销 Blob URL，下载 ID 已返回但浏览器可能尚未读取完字节，导致归档完成后下载失败 | P0：用户无法取得已成功生成的归档，且小包验收不稳定复现 | M10-R3-D1 保持 Blob URL 到 Chrome 下载终态；全量自动化通过并由用户复验 15.8 MB 级 ZIP 后关闭 |
