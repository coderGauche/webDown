...
下一步严格只执行 **M5-T3：实现受控并发队列**。

M5-T3 完成后必须：

1. 更新本文件中的任务勾选；
2. 建立可配置并发上限的通用异步队列，任何时刻运行任务数不得超过配置；
3. 保持输入与结果可追踪，单项失败不得造成队列失控或丢失其他结果；
4. 覆盖空队列、并发上限、任务成功/失败和稳定调度，不提前实现 M5-T4 取消或 M5-T5 重试策略。
