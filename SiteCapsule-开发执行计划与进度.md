...
> 建立日期：2026-07-22  
> 当前阶段：M3 当前页面 DOM 快照
> 当前状态：M3 已完成，等待开始 M4-T1
> 下一任务：M4-T1 提取 DOM 中的 `src`、`href`、`srcset`、`poster`
...
| 当前里程碑 | M4 资源发现与资源图 |
| 当前任务 | 无进行中任务 |
| 最近完成 | M3-T8 建立静态页和 SPA 测试 fixture |
| 下一任务 | M4-T1 提取 DOM 中的 `src`、`href`、`srcset`、`poster` |
| 阻塞项 | 无 |
...
下一步严格只执行 **M4-T1：提取 DOM 中的 `src`、`href`、`srcset`、`poster`**。

M4-T1 完成后必须：

1. 更新本文件中的任务勾选；
2. 从最终 DOM 中提取 `src`、`href`、`srcset` 和 `poster` 候选，并保留元素、属性与文档 base URL 来源信息；
3. 正确处理空值、相对 URL、绝对 URL、多个 `srcset` 候选和不承载资源的同名属性；
4. 为普通与边界属性组合编写测试，不提前解析 inline style、SVG 或 `<style>` 中的资源。
