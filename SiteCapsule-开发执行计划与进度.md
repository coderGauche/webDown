...
> 当前状态：M2-T7 已完成，等待开始 M2-T8
> 下一任务：M2-T8 为状态机、协议和存储编写测试
...
下一步严格只执行 **M2-T8：为状态机、协议和存储编写测试**。

M2-T8 完成后必须：

1. 更新本文件中的任务勾选；
2. 完成 M2 验收门禁并将 M3-T1 设为下一任务；
3. 复核状态机、协议、运行时校验、错误模型、schema 和仓储的成功/失败/恢复路径；
4. 不越过 M2-T8 提前实现 M3 DOM 快照。
