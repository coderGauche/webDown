...
> 当前状态：M10-R5 已将结构化清单、失败说明和离线报告接入真实运行时 ZIP
> 下一任务：M10-R6 使用多域复杂页执行真实 ZIP、断网外联和视觉回归
...
| 当前任务 | 无进行中任务 |
| 最近完成 | M10-R5 真实运行时 ZIP 六项元数据、SHA-256 和离线报告接入 |
| 下一任务 | M10-R6 使用多域复杂页执行真实 ZIP、断网外联和视觉回归 |
| 阻塞项 | 运行时 ZIP 已具备可读报告和可机读清单，但复杂多域站点的递归 CSS/脚本依赖、断网外联和视觉还原仍待 R6 真实证明；M10-T3 归档成功率 P0 及缺测指标继续阻止发布 |
...
| 编号 | 日期 | 状态 | 问题 | 影响 | 解除条件 |
|---|---|---|---|---|---|
| B-001 | 2026-08-03 | 阻塞 | 真实归档可能只有 `index.html`，大量关键 CSS、图片、字体和脚本仍指向公网，且可出现没有 ZIP 条目的本地引用 | P0：ZIP 无法可靠离线查看，“Completed”会形成错误成功反馈 | M10-R1、R2、R4、R6 通过真实下载 ZIP 和断网浏览器验收 |
| B-002 | 2026-08-03 | 已解除 | 运行时曾未生成 `_sitecapsule/report.html` 和完整资源清单，用户看不到失败、跳过、外链或删除原因 | R5 已使所有普通完成 ZIP 生成六项元数据、可读报告和资源 SHA-256 | 真实 Chromium 下载 ZIP 与全量敏感扫描 E2E 已校验；R7 重算公开基线报告率 |
| B-003 | 2026-08-03 | 修复待验 | 最终 DOM 曾带入第三方 consent/payment iframe、追踪脚本和其他浏览器扩展的注入 URL/节点；R3 已加入通用克隆 DOM 清理和结构化计数 | P1：污染归档、触发离线外联，并可能泄漏非目标页面状态 | M10-R6 使用真实 ZIP 证明无未解释外联和扩展请求后解除 |
| B-004 | 2026-08-03 | 二次修复待用户复验 | 大 ZIP 下载曾同时存在 Blob URL 过早撤销和点击后分块搬运耗尽用户激活两个时序缺陷 | P0：用户无法取得已成功生成的归档，小包验收不能覆盖真实大包时序 | D1 保持 Blob URL 到 Chrome 终态；D2 在按钮可用前预组装整包并输出 Chrome 中断枚举；用户以 20 MB 级 ZIP 复验后关闭 |
...
下一步严格只执行 **M10-R6：使用多域复杂页执行真实 ZIP、断网外联和视觉回归**。

M10-R6 完成后必须：

1. 更新本文件中的任务勾选；
2. 使用至少一个多域复杂公开页生成真实下载 ZIP，保存视觉和请求证据；
3. 在严格断网下打开归档，证明无未解释 HTTP/HTTPS、扩展协议或缺失本地资源请求；
4. 比对线上/离线关键视觉区域，如实记录递归 CSS/脚本依赖、Canvas/WebGL 和动态后端的降级。
