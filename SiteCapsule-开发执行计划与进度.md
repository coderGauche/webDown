...
> 建立日期：2026-07-22  
> 当前阶段：M3 当前页面 DOM 快照
> 当前状态：M3-T2 已完成，等待开始 M3-T3
> 下一任务：M3-T3 克隆并序列化最终 DOM
...
下一步严格只执行 **M3-T3：克隆并序列化最终 DOM**。

M3-T3 完成后必须：

1. 更新本文件中的任务勾选；
2. 在 Content 中克隆最终文档，不修改页面正在使用的 DOM；
3. 序列化结果保留 doctype、`html` 根元素和等待完成后的动态内容；
4. 为正常文档、缺失 doctype 和动态内容添加测试，不提前实现 M3-T4 敏感值清理。
