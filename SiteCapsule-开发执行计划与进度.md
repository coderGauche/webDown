...
> 建立日期：2026-07-22  
> 当前阶段：M3 当前页面 DOM 快照
> 当前状态：M3-T7 已完成，等待开始 M3-T8
> 下一任务：M3-T8 建立静态页和 SPA 测试 fixture
...
下一步严格只执行 **M3-T8：建立静态页和 SPA 测试 fixture**。

M3-T8 完成后必须：

1. 更新本文件中的任务勾选；
2. 建立可重复运行的普通静态 HTML fixture 与延迟渲染 SPA fixture，覆盖 M3 当前页面快照能力；
3. 验证元数据、最终 DOM、敏感值清理、特殊区域诊断和 Resource Timing 在两类页面中的稳定结果；
4. 完成 M3 里程碑验收门禁，不提前实现 M4-T1 的 DOM 资源属性扫描。
