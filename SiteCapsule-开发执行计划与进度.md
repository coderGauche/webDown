...
> 建立日期：2026-07-22  
> 当前阶段：M3 当前页面 DOM 快照
> 当前状态：M3-T1 已完成，等待开始 M3-T2
> 下一任务：M3-T2 实现可配置渲染等待
...
| 当前任务 | 无进行中任务 |
| 最近完成 | M3-T1 获取当前标签页 URL、标题、base URL 和最终 URL |
| 下一任务 | M3-T2 实现可配置渲染等待 |
| 阻塞项 | 无 |
...
下一步严格只执行 **M3-T2：实现可配置渲染等待**。

M3-T2 完成后必须：

1. 更新本文件中的任务勾选；
2. 使用 `CaptureSettings.renderWaitMs` 控制等待时间，并定义清晰的默认值与边界；
3. 等待发生在 Content 捕获上下文中，且不会阻塞 Side Panel 主线程；
4. 添加零等待、正常等待和非法等待值测试，不提前实现 M3-T3 DOM 序列化。
