...
- [x] **M7-T7** 使用 Chrome Downloads API 导出；
- [x] **M7-T8** 验证 ZIP CRC、解压和文件数量。
...
- [x] **M8-T6** 实现任务历史和本地清理；
- [x] **M8-T7** 实现中英文文案；
- [x] **M8-T8** 实现键盘操作、焦点和基础无障碍；
- [x] **M8-T9** 验证窄 Side Panel 下无文字溢出。
...
| 2026-08-05 | M10-R13-D1 | 缺陷修复，待真实页复验 | 用户重新归档后在 packaging 阶段得到 `Error.PathMapping`；确认 R13 首版把重定向前 URL 加入 `ResourcePathMapping.originalUrls`，改写器接受该别名，但清单校验仍要求每个 original URL 规范化后等于 final identity，形成跨模块契约冲突；清单现允许合法 HTTP/HTTPS 重定向别名，目录、文件名和路径仍严格绑定规范化 final URL，共享改写查找仍拒绝同一别名指向不同本地路径；新增从 `createRuntimeResourcePathPlan` 贯穿 `buildArchiveResourceManifests` 的 `@2 -> @2.0.2` 完整打包回归，不再只验证 JavaScript 改写半链路；定向 4 文件/33 项及全量 77 文件/680 项 Vitest、ESLint、Prettier、TypeScript 和 1.57 MB 生产构建通过 |
...
| 编号 | 日期 | 状态 | 问题 | 影响 | 解除条件 |
|---|---|---|---|---|---|
| B-001 | 2026-08-03 | 已解除 | 真实归档曾可能只有 `index.html`，大量关键 CSS、图片、字体和脚本仍指向公网，且可出现没有 ZIP 条目的本地引用 | R1-R6 已建立关键资源授权、最终 ZIP 强门禁、失败引用中和及真实断网验收 | R6 三个复杂公开页均生成真实 ZIP，通过引用审计，严格断网下外网、扩展协议和缺失本地请求均为 0 |
| B-002 | 2026-08-03 | 已解除 | 运行时曾未生成 `_sitecapsule/report.html` 和完整资源清单，用户看不到失败、跳过、外链或删除原因 | R5 已使所有普通完成 ZIP 生成六项元数据、可读报告和资源 SHA-256 | R7 固定基线的 21/21 个完成 ZIP 均包含报告与完整清单，报告率 100% |
| B-003 | 2026-08-03 | 已解除 | 最终 DOM 曾带入第三方 consent/payment iframe、追踪脚本和其他浏览器扩展的注入 URL/节点；R3 已加入通用克隆 DOM 清理和结构化计数 | R6 三站真实 ZIP 未出现外网、扩展协议或缺失本地资源请求 | 三站断网请求审计均为外网 0、扩展协议 0、缺失本地请求 0 |
| B-004 | 2026-08-03 | 已解除 | 大 ZIP 下载曾同时存在 Blob URL 过早撤销和点击后分块搬运耗尽用户激活两个时序缺陷 | D1 保持 Blob URL 到 Chrome 终态；D2 在按钮可用前预组装整包；2026-08-04 用户成功下载 24.8 MB United Carriers ZIP | 用户提供的解压目录含 448 个文件；442/442 清单资源均存在且大小、SHA-256 一致，离线首屏正常渲染 |
...
| D-104 | 2026-08-05 | 路径映射清单允许重定向请求别名，但路径身份和碰撞判断只使用规范化 final URL | 把 original URL 既当改写别名又要求它等于 final identity，会使重定向别名在脚本改写阶段有效、到最终清单阶段却触发 `Error.PathMapping` | 校验每个别名都是无凭据 HTTP/HTTPS URL，不要求其与 final URL 相等；路径目录、文件名、query/collision hash 仍只由 final identity 验证；同一别名映射多路径继续在共享查找表报错；回归必须贯穿最终清单生成 |
