...
> 建立日期：2026-07-22  
> 当前阶段：M5 下载引擎与权限策略
> 当前状态：M5-T5 已完成，等待开始 M5-T6
> 下一任务：M5-T6 记录重定向、final URL、状态码和 MIME
...
| 当前里程碑 | M5 下载引擎与权限策略 |
| 当前任务 | 无进行中任务 |
| 最近完成 | M5-T5 实现超时、重试和 Retry-After |
| 下一任务 | M5-T6 记录重定向、final URL、状态码和 MIME |
| 阻塞项 | 无 |
...
下一步严格只执行 **M5-T6：记录重定向、final URL、状态码和 MIME**。

M5-T6 完成后必须：

1. 更新本文件中的任务勾选；
2. 在资源请求结果中记录 original URL、final URL、重定向关系、HTTP 状态码和规范化响应 MIME；
3. 明确区分成功与非成功 HTTP 响应，保留可诊断元数据，不因元数据读取错误消费或丢失响应 body；
4. 覆盖无重定向/单次/多次重定向、缺失或非法 MIME、状态码边界和中止，不提前实现 M5-T7 单文件与任务总体积限制。
