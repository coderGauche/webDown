...
> 建立日期：2026-07-22  
> 当前阶段：M3 当前页面 DOM 快照
> 当前状态：M4-T1 已完成，等待开始 M4-T2
> 下一任务：M4-T2 提取 inline style、SVG 和 `<style>` 资源
...
| 当前里程碑 | M4 资源发现与资源图 |
| 当前任务 | 无进行中任务 |
| 最近完成 | M4-T1 提取 DOM 中的 `src`、`href`、`srcset`、`poster` |
| 下一任务 | M4-T2 提取 inline style、SVG 和 `<style>` 资源 |
| 阻塞项 | 无 |
...
下一步严格只执行 **M4-T2：提取 inline style、SVG 和 `<style>` 资源**。

M4-T2 完成后必须：

1. 更新本文件中的任务勾选；
2. 收集 inline `style`、`<style>` 文本和 SVG URL 承载位置，并保留所在元素、文档 URL 与 base URL 上下文；
3. 区分 CSS 文本源与 SVG 直接 URL 属性，跳过空内容及不承载资源的 SVG 引用；
4. 为普通、空值、SVG 命名空间和来源上下文编写测试，不提前实现 M4-T3 的 CSS AST URL 解析。
