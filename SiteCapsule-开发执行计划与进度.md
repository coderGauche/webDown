...
> 建立日期：2026-07-22  
> 当前阶段：M5 下载引擎与权限策略
> 当前状态：M5-T3 已完成，等待开始 M5-T4
> 下一任务：M5-T4 实现 AbortController 暂停、取消基础
...
下一步严格只执行 **M5-T4：实现 AbortController 暂停、取消基础**。

M5-T4 完成后必须：

1. 更新本文件中的任务勾选；
2. 使用 `AbortController`/`AbortSignal` 建立暂停与取消的中止基础，中止后不得启动新的队列项；
3. 将信号传给已启动工作项，并明确区分已完成、已启动后中止和未启动结果；
4. 覆盖启动前中止、运行中暂停/取消和竞态清理，不提前实现 M5-T5 超时与重试策略。
