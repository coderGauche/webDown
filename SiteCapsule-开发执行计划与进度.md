...
> 建立日期：2026-07-22  
> 当前阶段：M5 下载引擎与权限策略
> 当前状态：M5-T8 已完成，等待开始 M5-T9
> 下一任务：M5-T9 实现资源失败不阻断主任务
...
| 当前里程碑 | M5 下载引擎与权限策略 |
| 当前任务 | 无进行中任务 |
| 最近完成 | M5-T8 实现协议、origin 和本地网络安全策略 |
| 下一任务 | M5-T9 实现资源失败不阻断主任务 |
| 阻塞项 | 无 |
...
下一步严格只执行 **M5-T9：实现资源失败不阻断主任务**。

M5-T9 完成后必须：

1. 更新本文件中的任务勾选；
2. 将单资源成功、HTTP/网络失败、安全策略拒绝、超限和中止结果转换为稳定的逐资源结果，不因次要资源失败提前终止其余队列项；
3. 明确区分主文档致命失败、次要资源可继续失败，以及任务暂停/取消产生的中止和未启动状态；
4. 汇总成功、失败、中止和未启动计数并保留结构化错误，覆盖混合结果与并发乱序，不提前实现 M5-T10 多域 fixture 集成。
