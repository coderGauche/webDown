...
> 建立日期：2026-07-22  
> 当前阶段：M3 当前页面 DOM 快照
> 当前状态：M3-T4 已完成，等待开始 M3-T5
> 下一任务：M3-T5 记录 iframe、Shadow DOM 和不可访问区域
...
| 项目 | 当前值 |
|---|---|
| 当前里程碑 | M3 当前页面 DOM 快照 |
| 当前任务 | 无进行中任务 |
| 最近完成 | M3-T4 清理密码、Token 和敏感表单值 |
| 下一任务 | M3-T5 记录 iframe、Shadow DOM 和不可访问区域 |
| 阻塞项 | 无 |
...
下一步严格只执行 **M3-T5：记录 iframe、Shadow DOM 和不可访问区域**。

M3-T5 完成后必须：

1. 更新本文件中的任务勾选；
2. 为 iframe、开放 Shadow Root、关闭或不可访问区域建立稳定的诊断记录，不把无法访问误报为成功捕获；
3. 诊断信息只包含定位与原因等必要元数据，不泄露页面正文或敏感值；
4. 覆盖同源/跨源 iframe、开放/关闭 Shadow DOM 和无特殊区域页面测试，不提前实现 M3-T6 Performance Resource Timing 收集。
