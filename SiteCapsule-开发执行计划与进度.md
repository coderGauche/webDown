...
> 建立日期：2026-07-22  
> 当前阶段：M4 资源发现与资源图
> 当前状态：M4-T2 已完成，等待开始 M4-T3
> 下一任务：M4-T3 使用 CSSTree 解析 `url()`、`@import` 和 `@font-face`
...
| 当前里程碑 | M4 资源发现与资源图 |
| 当前任务 | 无进行中任务 |
| 最近完成 | M4-T2 提取 inline style、SVG 和 `<style>` 资源 |
| 下一任务 | M4-T3 使用 CSSTree 解析 `url()`、`@import` 和 `@font-face` |
| 阻塞项 | 无 |
...
下一步严格只执行 **M4-T3：使用 CSSTree 解析 `url()`、`@import` 和 `@font-face`**。

M4-T3 完成后必须：

1. 更新本文件中的任务勾选；
2. 使用 CSSTree AST 解析 M4-T2 收集的 CSS 原文，不使用正则表达式替代 CSS 语法解析；
3. 覆盖 `url()`、`@import` 和 `@font-face src`，保留源位置、字体格式及文档/base URL 上下文；
4. 对引号、转义、空白、data URL、fragment 和非法 CSS 编写测试，不提前实现 M4-T4 的资源合并。
