...
> 建立日期：2026-07-22  
> 当前阶段：M4 资源发现与资源图
> 当前状态：M4-T6 已完成，等待开始 M4-T7
> 下一任务：M4-T7 识别 data URL、Blob URL 和不支持协议
...
| 当前里程碑 | M4 资源发现与资源图 |
| 当前任务 | 无进行中任务 |
| 最近完成 | M4-T6 实现 `ResourceGraph` 及来源追踪 |
| 下一任务 | M4-T7 识别 data URL、Blob URL 和不支持协议 |
| 阻塞项 | 无 |
...
下一步严格只执行 **M4-T7：识别 data URL、Blob URL 和不支持协议**。

M4-T7 完成后必须：

1. 更新本文件中的任务勾选；
2. 对资源 URL 明确分类为 HTTP(S)、data、Blob 和不支持协议，分类结果必须确定且可严格校验；
3. data URL 保留可后续解码的信息，Blob URL 明确标记其页面会话限制，不支持协议不得进入后续网络下载队列；
4. 为大小写、空白、畸形 data URL、Blob URL、浏览器内部协议和自定义协议编写测试，不提前实现 M4-T8 的资源类型与 MIME 推断。
