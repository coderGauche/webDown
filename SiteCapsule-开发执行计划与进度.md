...
> 建立日期：2026-07-22  
> 当前阶段：M5 下载引擎与权限策略
> 当前状态：M5-T6 已完成，等待开始 M5-T7
> 下一任务：M5-T7 实现单文件和任务总体积限制
...
| 当前里程碑 | M5 下载引擎与权限策略 |
| 当前任务 | 无进行中任务 |
| 最近完成 | M5-T6 记录重定向、final URL、状态码和 MIME |
| 下一任务 | M5-T7 实现单文件和任务总体积限制 |
| 阻塞项 | 无 |
...
下一步严格只执行 **M5-T7：实现单文件和任务总体积限制**。

M5-T7 完成后必须：

1. 更新本文件中的任务勾选；
2. 在读取 body 前使用可信 `Content-Length` 预检单文件与剩余任务预算，且对缺失/伪造长度的流式响应在读取过程中强制截止；
3. 并发资源必须通过共享且原子的任务预算预留/提交/释放，失败、取消或超限不得泄漏预算，累计字节不得超限；
4. 覆盖精确上限、超限 1 字节、未知/错误 Content-Length、分块流、并发竞态、中止和无限制配置，不提前实现 M5-T8 协议/origin/本地网络安全策略。
