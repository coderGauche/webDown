...
> 当前状态：M8-T8 已完成并通过验收
> 下一任务：M8-T9 验证窄 Side Panel 下无文字溢出
...
下一步严格只执行 **M8-T9：验证窄 Side Panel 下无文字溢出**。

M8-T9 完成后必须：

1. 更新本文件中的任务勾选；
2. 以 Chrome Side Panel 可实际调整的窄宽度验证英文和中文界面，标题、按钮、文件名、URL、计数、错误、历史和诊断内容不得造成水平滚动或遮挡；
3. 使用明确的响应式约束、换行和溢出策略，保持固定尺寸控件和焦点边框可见，不通过缩小字号或隐藏必要内容规避问题；
4. 增加可自动验证的窄宽度布局边界，并由用户在 Chrome 中完成中英文和关键动态状态的窄面板验收；不提前实现站点爬取模式。
