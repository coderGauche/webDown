...
> 建立日期：2026-07-22  
> 当前阶段：M1 扩展工程骨架  
> 当前状态：M1-T1 已完成，等待开始 M1-T2  
> 下一任务：M1-T2 建立 Side Panel、Background、Content、Offscreen 入口  
> Git 基线：`master` / `1da7a5c first commit` / 已同步 `origin/master`
...
| M1 | 扩展工程骨架 | 2-3 工程日 | 进行中 |
| M2 | 任务模型、消息协议与持久化 | 2-3 工程日 | 待开始 |
...
- [ ] **M1-T2** 建立 Side Panel、Background、Content、Offscreen 入口；
- [ ] **M1-T3** 配置 Manifest V3 基础权限和图标占位；
- [ ] **M1-T4** 配置 ESLint、Prettier、Vitest 和类型检查；
- [ ] **M1-T5** 建立 `src/` 模块目录和路径别名；
- [ ] **M1-T6** 实现最小消息往返：Side Panel -> Background -> Content；
- [ ] **M1-T7** 编写开发、构建、测试和加载说明；
- [ ] **M1-T8** 在 Chromium 中加载未打包扩展并保存验证截图。
...
- [ ] **M2-T1** 定义 `CaptureJob`、`CaptureSettings`、`ResourceRecord`；
- [ ] **M2-T2** 定义任务状态机和合法状态转换；
- [ ] **M2-T3** 定义带版本号的消息协议；
- [ ] **M2-T4** 对跨上下文消息做运行时校验；
...
| Git 同步状态 | `master` 与 `origin/master` 一致 |
| 最近提交 | `1da7a5c first commit` |
| 最近验证日期 | 2026-07-22 |
...
下一步严格只执行 **M1-T2：建立 Side Panel、Background、Content、Offscreen 入口**。
