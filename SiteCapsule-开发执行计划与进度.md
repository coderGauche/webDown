...
> 建立日期：2026-07-22  
> 当前阶段：M3 当前页面 DOM 快照
> 当前状态：M3-T3 已完成，等待开始 M3-T4
> 下一任务：M3-T4 清理密码、Token 和敏感表单值
...
下一步严格只执行 **M3-T4：清理密码、Token 和敏感表单值**。

M3-T4 完成后必须：

1. 更新本文件中的任务勾选；
2. 仅清理克隆 DOM，不读取、覆盖或触发页面实时表单；
3. 从快照中移除密码字段、常见 Token/密钥字段和敏感表单当前值，同时保留非敏感页面结构；
4. 为密码、隐藏 Token、敏感字段命名和普通表单值边界添加测试，不提前实现 M3-T5 iframe、Shadow DOM 与不可访问区域记录。
