...
> 建立日期：2026-07-22  
> 当前阶段：M4 资源发现与资源图
> 当前状态：M4-T7 已完成，等待开始 M4-T8
> 下一任务：M4-T8 实现资源类型与 MIME 推断
...
| 当前里程碑 | M4 资源发现与资源图 |
| 当前任务 | 无进行中任务 |
| 最近完成 | M4-T7 识别 data URL、Blob URL 和不支持协议 |
| 下一任务 | M4-T8 实现资源类型与 MIME 推断 |
| 阻塞项 | 无 |
...
下一步严格只执行 **M4-T8：实现资源类型与 MIME 推断**。

M4-T8 完成后必须：

1. 更新本文件中的任务勾选；
2. 基于可靠证据建立资源类型与 MIME 推断结果，明确证据优先级和未知回退，不将文件扩展名猜测冒充响应 MIME；
3. 综合 DOM 标签/属性、CSS 上下文、Performance initiator、data header 和 URL 扩展名，在冲突时保留可解释证据；
4. 为脚本、样式、图片、字体、媒体、文档、JSON/WASM、无扩展名和冲突证据编写测试，不提前执行 M4-T9 的综合复杂 URL 测试任务。
